...
export default function BusinessList() {
  ...
  const fetchData = () => api.get("/business-managers").then((r) => setData(r.data));
  ...
  const handleDelete = async (id: number) => {
    await api.delete(`/business-managers/${id}`);
    message.success("已删除");
    fetchData();
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        <h2 style={{ fontSize: 20, fontWeight: 600, color: "#e6edf3" }}>商务人员管理</h2>
      ...
      <Modal title="添加商务人员" open={open} onOk={handleSubmit} onCancel={() => setOpen(false)} okText="确认" cancelText="取消">
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="姓名" rules={[{ required: true, message: "请输入姓名" }]}>
            <Input placeholder="商务人员姓名" />
          </Form.Item>
